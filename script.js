const CONFIG = {
  googleAppsScriptUrl:
    "https://script.google.com/macros/s/AKfycbxMhlg6kitG02B0-8LZfhi3SOf9eBLz6qRuxOKFlJinzvXurVDeffTYAruoLcV3I20tbQ/exec",
...
async function showWorkspace() {
  ...
  setText("#userChip", `${initials(state.user.name)}  ${state.user.name}`);
  setText("#welcomeTitle", `Welcome back, ${state.user.name}`);
  setText("#profileAvatar", initials(state.user.name));
  setText("#profileName", state.user.name);
  setText("#profileEmail", state.user.email);
  setText("#profileMeta", `User ID: ${state.user.userId}`);
  await refreshData();
...
function renderLoadError(message) {
  ...
  $("#dashboardNotes").innerHTML = appError(`Google Sheet data load nahi ho paya. ${message}`);
  $("#testList").innerHTML = appError(message);
  $("#noteList").innerHTML = appError(message);
  $("#resultHistory").innerHTML = appError(message);
  if ($("#recentResults")) $("#recentResults").innerHTML = appError(message);
  setText("#syncState", "Connection error");
}

function renderAll() {
  const activeTests = state.tests.filter((test) => test.status === "Active");
  const activeNotes = state.notes.filter((note) => note.status === "Active");
  const unlockedItems = [...activeTests, ...activeNotes].filter((item) => !isPaid(item) || item.isUnlocked).length;
  const best = state.results.length ? Math.max(...state.results.map((item) => Number(item.percentage || 0))) : 0;

  setText("#availableCount", String(activeTests.length));
  setText("#notesCount", String(activeNotes.length));
  setText("#attemptedCount", String(state.results.length));
  setText("#unlockedCount", String(unlockedItems));
  setText("#heroBestScore", `${best}%`);
